...
export class FuseNavVerticalCollapsableComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        if(location.href.split('/').pop()){
            if ( this.navigation.find(item => item.linkUrl && item.linkUrl.indexOf("/" + location.href.split('/').pop()) != -1) && this.item.id == this.navigation.find(item => item.linkUrl && item.linkUrl.indexOf("/" + location.href.split('/').pop()) != -1).chucNangChaId)
            {
                this.expand();
            }
            else
    ...
    filterItemChild(idParent:any){
        return this.navigation.filter(item => item.chucNangChaId == idParent);
    }
